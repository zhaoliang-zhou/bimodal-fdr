...
# Public reproducibility script for the empirical-Bayes auxiliary-information
# analyses, using the numerical estimates reported in the manuscript.
#
# Usage from this directory:
#   Rscript reproduce_results_from_published_estimates.R
#   Rscript reproduce_results_from_published_estimates.R --output=results
#
# This public version intentionally does not load or distribute the restricted
# LLD data. LLD-derived parameter estimates and reported summary values are
# recorded below as fixed inputs. The script writes model-based figures and
# numerical checks to the output directory.
...
if (is.null(output_dir)) {
  output_dir <- file.path(script_dir, "..", "results_from_published_estimates")
}
...
# These are fixed numerical inputs from the current manuscript analysis.
# They replace the private-data loading and moment-estimation steps in the
# original script. Full precision is retained so downstream calculations are
# reproducible from the reported analysis inputs.
pi0 <- 0.93
pi0_t2 <- 0.9083
mu1 <- 2.08303076431596
alpha0_mom <- 2.12476855383552
beta0_mom <- 2.71716157952177
alpha1_mom <- 1.14938974126148
beta1_mom <- 2.16642552125939
alpha_s_mom <- 1.93992871236498
beta_s_mom <- 2.18087418382892
alpha_bimodal_mom <- 2.03890859876801
alpha_level <- 0.20
...
save_plot <- function(plot_object, filename, width = 7, height = 5, dpi = 350) {
  ggplot2::ggsave(
...
t_grid <- seq(
  mu1 + 1e-6,
  max(
    stats::qgamma(0.999, shape = alpha0_mom, rate = beta0_mom),
    mu1 + stats::qgamma(0.999, shape = alpha_bimodal_mom, rate = beta1_mom)
  ),
  length.out = 2000
)
lr_data <- data.frame(
...
# These summaries were obtained from the restricted LLD analysis and are
# included as fixed reported values; no individual LLD statistics are needed.
thresholds <- data.frame(
  model = c("FC-only", "FC-plus-SC"),
  threshold = c(0.226305830375733, 0.357353215148844),
  number_rejected = c(47L, 160L),
  average_lfdr = c(0.199895075578629, 0.199453312319234)
)
...
# The conditional Storey/GAM fit requires the individual LLD p-values and SC
# statistics, so it is intentionally not rerun in this public script. Its
# reported summaries are retained as fixed values for transparent provenance.
pi0_storey_marginal <- 0.908313285217856
pi0_sc_storey <- 0.908313285217857
utils::write.csv(
...
summary_lines <- c(
  "Raw LLD data: not included; reported LLD summaries are fixed inputs.",
  paste0("FC-only threshold: ", signif(thresholds$threshold[[1L]], 8)),
  paste0("FC-only rejections: ", thresholds$number_rejected[[1L]]),
  paste0("FC-plus-SC threshold: ", signif(thresholds$threshold[[2L]], 8)),
  paste0("FC-plus-SC rejections: ", thresholds$number_rejected[[2L]]),
